...
class AddNewerAreaToRasterFunction < ActiveRecord::Migration
  ...
  def change
    ...
    RETURNS table (nodata_value numeric, densifier integer, srid integer, final_image boolean, image_path text, data_type text) AS
        ...
        def raster_metadata(raster_variable_id):
            ...
            query  = "select id, mnemonic, area_reference_id, second_area_reference_id, mnemonic_type from rasters_metadata_view where id = %s" % (raster_variable_id)
        ...
        def get_area_data_values(sample_geog_level_id, area_data_id, raster_data_tables):
            ...
            if variable_type == 'Count':
                ...
                (
                SELECT bound.code::integer as id, bound.description as name, ST_Transform(bound.geog::geometry, p.srid) as geometry
                ...
                (
                SELECT bound.code::integer as id, bound.description, adv.mnemonic, area_data.value, ST_AsText(bound.geog::geometry) as geometry
                ...
                query  = '''SELECT bound.code::integer as id, bound.description, adv.mnemonic, area_data.value:: double precision as value, ST_AsText(bound.geog::geometry) as geometry
                FROM sample_geog_levels sgl
                inner join geog_instances gi on sgl.id = gi.sample_geog_level_id
                inner join boundaries bound on bound.geog_instance_id = gi.id
                inner join area_data_values area_data on area_data.geog_instance_id = gi.id
        ...
        densifier_list = determine_raster_densification(shape_path, metadata, metadata["geoProj"], num_features, area_data_type)
