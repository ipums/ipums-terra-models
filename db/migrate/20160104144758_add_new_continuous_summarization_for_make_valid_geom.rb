...
class AddNewContinuousSummarizationForMakeValidGeom < ActiveRecord::Migration

  def change
    
    sql = <<-SQL
      CREATE OR REPLACE Function terrapop_continuous_summarization1(sample_geog_lvl_id bigint, rasters_id bigint)
            ...
            BEGIN
                ...
                contin_rast as 
                    ...
                    SELECT sgl.id as sample_geog_level_id, gi.id as geog_instance_id, gi.label as geog_instance_label,
                      gi.code as geog_instance_code, ST_Union(ST_Clip(r.rast, ST_Transform(ST_MakeValid(bound.geog::geometry), (SELECT ST_SRID(r.rast) FROM rasters r  WHERE r.raster_variable_id = rasters_id LIMIT 1)))) as rast
                      FROM sample_geog_levels sgl
                      inner join geog_instances gi on sgl.id = gi.sample_geog_level_id
                      inner join boundaries bound on bound.geog_instance_id = gi.id
                      inner join rasters r on ST_Intersects(r.rast, ST_Transform(ST_MakeValid(bound.geog::geometry), (SELECT ST_SRID(r.rast) FROM rasters r  WHERE r.raster_variable_id = rasters_id LIMIT 1)))
