...
class CorrectedNetCdfAsRasterFunction < ActiveRecord::Migration
  ...
  def change
    ...
    RETURNS table (nodata_value numeric, srid integer, final_image boolean, image_path text, data_type text) AS
        ...
        def GetCoverage(coverageName, maxX, maxY, minX, minY, width, height,imageType,userDate, outTiff):
            ...
            coverage_url = 'http://geoserver/geoserver/wcs?service=WCS&request=GetCoverage&version=1.0.0&coverage=%s&BBOX=%s,%s,%s,%s&Width=%s&Height=%s&CRS=EPSG:4326&TIME=%s&format=%s' % (coverageName,maxX,maxY,minX,minY,width,height,userDate,imageType)
